...
#         # 初始化订单面板
#
#     def init_confirm_order(self):#bottom bar
#         #结算按钮
#         checkout_button = tk.Button(self.confirm_panel, text="Place the order", command=self.checkout_window, width=30, height=3)
#         checkout_button.pack(side="bottom", expand=True)
#         # temporary setting up table button
#         temp_button = tk.Button(self.confirm_panel, text="Table Confirmation", command=self.temp_button, width=30, height=3)
#         temp_button.pack(side="bottom", expand=True)
#         #real-time price display
#         self.sum_label = tk.Label(self.confirm_panel, text="Sum: "+str(self.total_price), font=("Arial", 16), bg="white")
#         self.sum_label.pack(side="bottom", expand=True)
#
...
#         #重要！临时措施，后面这里接入table，点击table确定落座后，创建订单实例
#         from Controllers.OrderController import OrderController
